...
# """
# ============================================================================
# Apr-22 Phase 4 prompt caching (Analyses/2026-04-22_prompt-caching-implementation-plan.md):
# ============================================================================
# The original PERSONA_TEMPLATE was rendered as one ~8000-char HumanMessage via LangChain's
# PromptTemplate. Caching was impossible — everything went in a single user message, no system
# block, no cache_control breakpoint.
#
# PERSONA_SYSTEM_TEMPLATE below contains the stable rules (role detection, role-aware output,
# fallback behavior, 9-field persona generation spec, ANONYMIZATION-FRIENDLY PROSE rule, the
# PUBLIC PRIVACY RULE + CONSISTENCY MANDATE + SELF-CHECK from Apr-19 F/u 28 2290062 + F/u 29
# 053fc68, the EXCEPTION for persona_archetype/persona_designation) and the JSON output
# format + output-format enforcement. Everything in here is byte-identical across all
# persona generation calls site-wide — ONE cached prefix shared across ALL users. Content
# preserved verbatim from the original PERSONA_TEMPLATE; only structural placement moves
# (the trailing "CRITICAL OUTPUT INSTRUCTION" block is promoted into the system template
# so the cached prefix is fully complete, and a brief recency-effect reminder goes in the
# user message instead).
#
# PERSONA_USER_TEMPLATE: minimal per-user wrapper. Contains the combined_data (full
# onboarding conversation + extracted Q&A + resume) plus a short output-format reminder
# that preserves the recency prompt-engineering effect. This is the only content that
# varies per call.
#
# Privacy invariants preserved byte-identical:
#   I-5 PUBLIC PRIVACY RULE (lines 160-183 of pre-refactor PERSONA_TEMPLATE)
#   I-5 CONSISTENCY MANDATE (lines 185-195)
#   I-5 SELF-CHECK (lines 197-205)
#   I-6 EXCEPTION for persona_archetype + persona_designation (lines 207-210)
#   Apr-19 F/u 29 `f708dd2` experience-field privacy extension (line 168)
#
# Any drift from the privacy rules in post-deploy persona output → immediate rollback.

PERSONA_SYSTEM_TEMPLATE = """You are an expert persona generation system. Using the combined data provided in the user message (user questions + resume text + full onboarding conversation), generate THREE structured outputs: a professional persona, their requirements, and their offerings.
...
# Per-user variable content for the user message. combined_data is the only per-call variable.
PERSONA_USER_TEMPLATE = """Combined Input Data:
{combined_data}

Output JSON only, matching the schema in your system prompt. Start directly with {{ and end with }}."""


# Legacy template kept for backward compatibility with any callers still using the
# old LangChain build_persona_chain path. The Phase 4 refactor uses PERSONA_SYSTEM_TEMPLATE
# + PERSONA_USER_TEMPLATE via call_with_fallback. If you're reading this and nothing in
# the codebase imports PERSONA_TEMPLATE anymore (check with grep), this can be removed.
PERSONA_TEMPLATE = """
